...
- Policy per (user, invite code): first channel → in-channel warning reply on every offending message; second **distinct** channel → post one alert to the moderation channel (hardcoded `ALERT_CHANNEL_ID = 996386287059742851`, guild 981525647891525642) and delete all recorded offending messages, plus every later repeat for that pair. Failed deletes trigger an in-channel notice at most **once per user** across all channels.
