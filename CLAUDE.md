...
- Persistent state: `$XDG_CONFIG_HOME/discord_to_insta/state.json` (falls back to `~/.config/…`). Schema: `{ last_reacted_by_channel: { channel_id: message_id }, reactions_done_by_channel: { channel_id: { message_id: [emoji, …] } } }`.
